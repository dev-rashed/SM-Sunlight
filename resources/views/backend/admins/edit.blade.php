  ...
  <div class="card-body">
    <form action="{{ route('administrative.update', $admin->id) }}" method="POST">
      ...
      <div class="row mb-3">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control" value="{{ $admin->user_name }}" name="user_name" id="user_name" disabled placeholder="username_here" />
        </div>
      </div>
      <div class="row mb-3">
      ...
      <div class="row mb-3">
        ...
        <div class="col-sm-10">
            <input type="number" id="basic-default-phone" value="{{ $admin->phone }}" name="phone" class="form-control"/>
        </div>
      </div>
      <div class="row mb-3">
        ...
        <div class="col-sm-10">
          <select name="store" id="store" class="form-control">
            @foreach ($stores as $store)
                <option value="{{ $store->id}}" {{ $admin->store_id && $admin->store_id == $store->id ? 'selected' : "" }}>{{ $store->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
